...
export const PATCH = asyncWrapperAuthorisedApi(async (req , userInfo)=>{ 
    ...
    if(!cityInfo.dislikes.includes(userInfo.userId)) return apiResponse(StatusCodes.BAD_REQUEST , errorMessage("City is not dislike yet!"))
    const newCityInfo = await cityModal().findOneAndUpdate({name :city } , { dislikes :  cityInfo.dislikes.filter((item : string )=>item !== userInfo.userId)  } , {new:  true} )
