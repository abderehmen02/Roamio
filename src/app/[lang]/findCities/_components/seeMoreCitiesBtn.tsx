...
export const SeeMoreCities : React.FC = ()=>{
    ...
    const pageEnd : boolean = Boolean(searchParams.get(QueryObjParams.endPage) === "true" ) 
...
{     <SecondaryBtn onClick={prevPage} className={cn({"invisible " : Number(currentPage) < 2 })} >Previous <i className="bi mx-3 text-2xl bi-arrow-left-circle-fill"></i> </SecondaryBtn> }
{     <PrimaryBtn onClick={nextPage} className={cn({"invisible" : pageEnd})} >Next <i className="bi text-2xl mx-3 bi-arrow-right-circle-fill"></i> </PrimaryBtn>  }
     </div> }
     </div>
}