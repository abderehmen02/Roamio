...
//functions      
   ...
   {isCityDb(city) && Boolean(city.places.length) && <PlacesModal categories={city.categories.map(category=>category.name)} places={city.places}  openPlacesModal={openPlacesModal} setOpenPlacesModal={setOpenPlacesModal} /> }
    ...
    <img alt={subtitle} src={image}  style={{ minHeight: '200px' , objectFit: 'cover' ,  }} className={ cn( "w-full laptop:w-[300px] rounded-l-xl border-2 " , {"h-full" : !seeAllDescreption && !seeMoreInfo  , "h-fit " : seeAllDescreption || seeMoreInfo } )} />
    <div className="flex px-6 w-full py-1 justify-around flex-col " >
     <Title  title={cityWikipediaData.title}  titleClassName="  text-2xl" className="laptop:flex-row items-start   laptop:items-center  justify-start  w-fit laptop:gap-7"  descreptionClassName="font-bold text-secondaryDark  text-start" descreption={subtitle}  />
