...
{featuresSectionInfo.map((sectionInfo , index )=><div key={sectionInfo.longTitle} className={cn("cursor-pointer w-full px-4 py-4  laptop:w-full shadow-xl bg-white rounded-2xl border-2 border-black hover:bg-whiteYellwed laptop:px-16" , {"bg-whiteYellwed" : index === selectedFeatureIndex , "bg-white" : index !== selectedFeatureIndex })} onClick={()=>setSelectedFeatureIndex(index)}><FeatureSectionCard title={sectionInfo.shortTitle} descreption={sectionInfo.shortDescreption} iconSrc={sectionInfo.srcIcon}  /></div>)}
</div>
